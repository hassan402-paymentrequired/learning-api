...
import { Users, BookOpen, FileQuestion, Activity, TrendingUp, Clock, Award, Target } from 'lucide-react';
...
export default function Dashboard({ stats, recentAttempts, recentUsers, recentExams, topExams, subjectPerformance }: Props) {
    return (
            ...
            <div className="flex h-full flex-1 flex-col gap-4 p-4">
                ...
                <div className="grid gap-4 md:grid-cols-2">
                    <Card>
                        ...
                        <CardContent>
                            <div className="text-4xl font-bold">{stats.average_score.toFixed(1)}%</div>
                                    ...
                                    topExams.map((exam, index) => (
                                        <div key={exam.id} className="flex items-center justify-between">
                                            ...
                                            <div className="flex items-center gap-4">
                                                ...
                                                <span className="text-sm font-medium">{exam.avg_score.toFixed(1)}%</span>
                                            </div>
                                        </div>
                                    ))
                                ) : (
                ...
                <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-3">
                    <Card>
                        <CardHeader>
                            ...
                            <CardDescription>Latest completed attempts</CardDescription>
                        </CardHeader>
                        <CardContent>
                            <div className="space-y-3">
                                {recentAttempts.length > 0 ? (
                                    recentAttempts.map((attempt) => (
                                        <div key={attempt.id} className="flex items-center justify-between border-b pb-2 last:border-0">
                                            <div>
                                                <p className="text-sm font-medium">{attempt.user_name}</p>
                                                <p className="text-xs text-muted-foreground">{attempt.exam_title}</p>
                                            </div>
                                            <div className="text-right">
                                                <p className="text-sm font-medium">{attempt.percentage.toFixed(1)}%</p>
                                                <p className="text-xs text-muted-foreground">{attempt.completed_at}</p>
                                            </div>
                                        </div>
                                    ))
                                ) : (
                                ...
                                {recentUsers.length > 0 ? (
                                    recentUsers.map((user) => (
                                        <div key={user.id} className="flex items-center justify-between border-b pb-2 last:border-0">
                                            ...
                                            <p className="text-xs text-muted-foreground">{user.created_at}</p>
                                        </div>
                                    ))
                                ) : (
                    ...
                    <CardHeader>
                        <div className="flex items-center justify-between">
                            <div>
                                ...
                                <CardDescription>Latest added practice sets</CardDescription>
                            </div>
                            <Link href={admin.exams.index().url} className="text-sm text-primary hover:underline">
                                View all
                            </Link>
                        </div>
                    </CardHeader>
                        ...
                        <div className="space-y-3">
                            {recentExams.length > 0 ? (
                                recentExams.map((exam) => (
                                    <div key={exam.id} className="flex items-center justify-between border-b pb-2 last:border-0">
                                        <div>
                                        ...
                                        <p className="text-xs text-muted-foreground">{exam.created_at}</p>
                                    </div>
                                ))
                            ) : (
